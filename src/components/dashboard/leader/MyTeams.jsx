  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm max-w-xl">
        <h3 className="text-xl font-bold text-slate-900 mb-2">➕ Create New Team</h3>
        <p className="text-xs text-slate-500 mb-4">Create a workspace team and invite developers via email.</p>
        ...
        <form onSubmit={handleCreateTeam} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-[var(--color-zoom-blue)] text-white font-bold py-3 rounded-xl text-sm shadow-md hover:bg-blue-700 transition-all">
            Launch Team
          </button>
        </form>
      </div>
      ...
      <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
        <h3 className="text-xl font-bold text-slate-900 mb-4">Your Created Teams</h3>
        
        {/* Agar koi team nahi banayi, toh message dikhao */}
        {teams.length === 0 ? (
          <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-300">
            <p className="text-sm text-slate-500 font-medium">No teams created yet. Create your first team above!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {teams.map(t => (
              <div key={t.id} className="p-6 rounded-2xl border border-slate-100 bg-slate-50 flex flex-col justify-between space-y-4">
                    ...
                    {(!t.membersList || t.membersList.length === 0) ? (
                      ...
                      <div className="space-y-1.5">
                        {t.membersList.map((m, idx) => (
                          <div key={idx} className="flex justify-between items-center bg-white px-3 py-2 rounded-xl border border-slate-200 text-xs">
                            <span className="font-medium text-slate-700">{m.email}</span>
                            <span className="px-2 py-0.5 rounded-md font-bold text-[10px] bg-amber-50 text-amber-600">
                              {m.status}
                            </span>
                          </div>
                        ))}
                  ...
                  {selectedTeamId === t.id ? (
                    <form onSubmit={(e) => handleInviteMember(e, t.id)} className="space-y-2 border-t border-slate-200 pt-3">
                      ...
                      <div className="flex gap-2">
                        <button type="submit" className="flex-grow bg-slate-900 hover:bg-black text-white font-bold py-2 rounded-xl text-xs transition-colors">
                          Send Invite
                        </button>
                        <button type="button" onClick={() => setSelectedTeamId(null)} className="px-4 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-xl text-xs transition-colors">
                          Cancel
                        </button>
                      </div>
                    </form>
                    ...
                    <button 
                      ...
                      className="w-full bg-white hover:bg-slate-100 text-slate-800 font-bold py-2.5 rounded-xl text-xs border border-slate-200 transition-colors shadow-sm"
                    >
                      + Add Member by Email
                    </button>
                  )}
